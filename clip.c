/* #ident	"@(#)x11:contrib/clients/xloadimage/clip.c 6.13 94/07/29 Labtam" */
/* clip.c:
 *
...
Image *clip(iimage, clipx, clipy, clipw, cliph, imgopp)
     Image          *iimage;
     int             clipx, clipy;
     ...
     ImageOptions   *imgopp;
{
  ...
  if (border_shows) {
	...
	if (border_pv >=  simage->rgb.size) {	/* can't fit another */
	  compress(simage, FALSE);		/* try compressing image */
	  border_pv = simage->rgb.used;
        }
