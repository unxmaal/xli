/* #ident	"@(#)x11:contrib/clients/xloadimage/image.h 6.15 93/07/23 Labtam" */
/* image.h:
 *
...
/* the device independent equivalent of the X Color record */
typedef struct {
	unsigned long pixel;
...
#define colorIntensity(R,G,B) \
  (RedIntensity[((unsigned short)(R)) >> 8] + GreenIntensity[((unsigned short)(G)) >> 8] + BlueIntensity[((unsigned short)(B)) >> 8])
