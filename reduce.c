/* #ident	"@(#)x11:contrib/clients/xloadimage/reduce.c 6.20 94/07/29 Labtam" */
/* reduce.c:
 *
...
#if defined(MSDOS) && defined(DJGCC_BUGS)
/* djgcc 1.09 has a bug that causes MAXDOUBLE
   to turn into a NAN on some occations.
   The following value is safe. */
#undef HUGE
...
static boolean  CutBoxes();
static int  CutBox();
static int  GreatestVariance();
static boolean  FindCutpoint();
static void CopyToNewImage();
...
Image *reduce(image, colors, ditherf, gamma, verbose)
Image *image;
unsigned int colors;
unsigned int ditherf;	/* if True, apply color disthering */
float gamma; 			/* if != 0.0, compensate for gamma post processing */
 ...
 */
static void
ComputeRGBMap(boxes, colors, rgbmap, ditherf)
Box *boxes;
int colors;
unsigned short *rgbmap;
int ditherf;
{
	...
	if (!ditherf) {
		 ...
		 */
		bfill (rgbmap, ColormaxI * ColormaxI * ColormaxI * sizeof(*rgbmap), 0xff);
...
NN *nnp;
	{
	long bdist, mdist;
	Intensity *red = outimage->rgb.red,
...
unsigned short
find_nearest(rval,gval,bval, nna, outimage)
int rval, gval, bval;
Image *outimage;
NN *nna;
	{
...
static void
CopyToNewImage( inimage, outimage, rgbmap, ditherf, colors, gamma, verbose)
Image *inimage,*outimage;
unsigned short *rgbmap;		/* pixel value lookup map */
int ditherf,colors;
float gamma;
int verbose;
{
  ...
  if(!ditherf)  {
	...
	if (gamma == 0.0 || (gamma/inimage->gamma) == 1.0)
	...
	for (x = NNmaxI * NNmaxI * NNmaxI, nnp = nna; x > 0; x--, nnp++)
    {
      if(nnp->length != 0)
      {
        lfree((char *)nnp->red);	/* free red, green and blue */
        lfree((char *)nnp->pixel);
      }
    }
	lfree((char *)nna);
  }
...
Image *expandbittoirgb(image, depth)
     Image *image;
     int    depth;
{ 
  Image *new_image;
...
Image *expandirgbdepth(image, depth)
     Image *image;
     int    depth;
{ 
  Image *new_image;
